...
public class LoadAgentsTest
{
    ...
    public void Cleanup()
    {
        ...
        {
            TestContext.WriteLine("TestCleanup directory removal deferred: {0}", exception);
        }
    }
    /// <summary>
    ...
    {
        string databaseDirectory = Path.Combine(Path.GetTempPath(), "idds_mail_seq_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(databaseDirectory);
        Database database = new();
        database.Configure(databaseDirectory, "mail-agents-seq.db");
        ...
        {
            CloseDatabaseAndDeleteDirectory(database, databaseDirectory);
        }
    }
    ...
    private static string FindBuiltPluginDirectory()
    {
        ...
        string pluginDirectory = Path.Combine(root.FullName, "src", "IDDSCommunity.IntrusionDetection.Admin", "bin", configuration, frameworkDirectory.Name, "Plugins");
