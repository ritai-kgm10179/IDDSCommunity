...
public class LanguageManagerTest
{
    ...
    {
        string root = FindRepositoryRoot();
        Regex forbidden = new("throw new [A-Za-z0-9_.<>]+Exception\\(\\s*\"|MessageBox\\.Show\\(\\s*\"|Console\\.Write(?:Line)?\\(\\s*\"|WindowsLogManager\\.Instance\\.WriteEntry\\(\\s*\"|EventMessage\\s*=\\s*\"|SetToolTip\\([^,]+,\\s*(?:string\\.Format\\()?\\s*\"", RegexOptions.Compiled);
