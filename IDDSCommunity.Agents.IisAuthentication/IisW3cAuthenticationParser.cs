...
internal sealed class IisW3cAuthenticationParser
{
    ...
    internal AuthenticationFailureEvent? Parse(string line)
    {
        if (line.StartsWith("#Fields:", StringComparison.OrdinalIgnoreCase)) { fields = line[8..].Trim().Split(' ', StringSplitOptions.RemoveEmptyEntries); return null; }
        ...
        row.TryGetValue("cs-uri-stem", out string? path);
        if (protectedPaths.Length > 0 && !protectedPaths.Any(prefix => (path ?? string.Empty).StartsWith(prefix, StringComparison.OrdinalIgnoreCase))) return null;
        return new AuthenticationFailureEvent(occurredAt, address, 401, "IIS", account ?? string.Empty, path ?? string.Empty);
    }
}
