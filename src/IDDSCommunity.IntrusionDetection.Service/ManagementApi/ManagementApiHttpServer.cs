    ...
    private async Task ProcessRequestAsync(HttpListenerContext context)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(expectedKey))
            {
                ...
                {
                    await SendJsonResponseAsync(response, HttpStatusCode.Unauthorized, new { error = "Unauthorized: Invalid API Key" });
                    return;
                }
            }
            ...
            if (method == "GET" && path is "/api/v1/status" or "/status")
            {
                var statusObj = new
                {
                    status = "healthy",
            ...
            if (method == "GET" && path == "/api/v1/locks")
            {
            ...
            if (method == "POST" && path == "/api/v1/locks")
            {
            ...
            if (method == "DELETE" && path.StartsWith("/api/v1/locks/"))
            {
            ...
            if (method == "GET" && path == "/api/v1/whitelist")
            {
        ...
        {
            await SendJsonResponseAsync(response, HttpStatusCode.InternalServerError, new { error = ex.Message });
        }
        finally
        {
