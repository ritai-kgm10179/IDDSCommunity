...
IDDS Community 的資料庫維護分為自動維護與必須停用服務的人工維護，所有成功操作都會寫入 `ProtectionAuditLog`。
...
本機備份用於快速復原，並不取代異地備份。正式環境應將已驗證備份複製到具備存取控制、加密、不可變保存與復原演練的外部儲存空間；維護中心不會自行管理外部儲存的權限或加密金鑰。
