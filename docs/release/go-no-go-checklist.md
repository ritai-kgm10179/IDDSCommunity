# IDDSCommunity 發布 Go／No-Go 檢查表

## 自動化門檻

- `dotnet build IDDSCommunity.slnx` 必須為 0 警告、0 錯誤。
- `dotnet test IDDSCommunity.slnx` 不得有失敗；略過測試必須逐項說明環境原因。
...
- `iddscommunity-runtime` 健康檢查至少為 Healthy；Degraded 必須有核准的已知原因。
- 監看 `IDDSCommunity.IntrusionDetection` Meter 的 received、dispatched 與 dropped 指標。
