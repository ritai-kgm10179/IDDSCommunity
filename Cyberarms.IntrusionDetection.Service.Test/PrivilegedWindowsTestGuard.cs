...
internal static class PrivilegedWindowsTestGuard
{
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("CYBERARMS_RUN_PRIVILEGED_TESTS"), "1", StringComparison.Ordinal))
            Assert.Inconclusive("Set CYBERARMS_RUN_PRIVILEGED_TESTS=1 to run privileged Windows integration tests.");
