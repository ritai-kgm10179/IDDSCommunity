...
public sealed class PluginDeploymentTest
{
    ...
    public void AdminBuildIncludesEveryProductionAgentPlugin()
    {
        ...
        XDocument adminProject = XDocument.Load(Path.Combine(root, "IDDSCommunity.IntrusionDetection.Admin", "IDDSCommunity.IntrusionDetection.Admin.csproj"));
    ...
    public void AdminOutputContainsEveryLoadableProductionAgentPlugin()
    {
        ...
        string pluginDirectory = Path.Combine(root, "IDDSCommunity.IntrusionDetection.Admin", "bin", configuration, targetFrameworkDirectory.Name, "Plugins");
        string[] expected = GetProductionAgentProjectNames(root);
    ...
    {
        string root = FindRepositoryRoot();
        string source = File.ReadAllText(Path.Combine(root, "IDDSCommunity.IntrusionDetection.Shared", "AgentLoaderProxy.cs"));
    ...
    {
        string root = FindRepositoryRoot();
        string adminDirectory = Path.Combine(root, "IDDSCommunity.IntrusionDetection.Admin");
