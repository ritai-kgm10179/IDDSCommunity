...
class Program
{
    ...
    static void Main(string[] args)
    {
        try
        {
            ...
            Console.WriteLine(Strings.Get("IDDSCommunity Intrusion Detection Command line plugin test tool"));
            var p = System.Diagnostics.Process.GetCurrentProcess();
            string executablePath = p.MainModule?.FileName ?? Environment.ProcessPath ?? string.Empty;
            Agents.Load(executablePath[..executablePath.LastIndexOf('\\')] + "\\Plugins\\IDDSCommunity.IntrusionDetection.Base.Plugins.dll");
    ...
    static void ShowUsage()
    {
        ...
        Console.WriteLine(Strings.Get("IDDSCommunityIdsCmd -assemblyName=assembly-to-load.dll -startAgent=agent1 [-startAgent=agent2]"));
        Console.WriteLine(Strings.Get(""));
        Console.WriteLine(Strings.Get("(c) 2012 IDDSCommunity, isiCore"));
    }
    /// <summary>
    ...
    class LogAlerts : List<LogAlert>
    {
        ...
        public LogAlert GetAlertFor(string ipAddress)
        {
            ...
            throw new InvalidOperationException(string.Format(IDDSCommunity.IntrusionDetection.Shared.Localization.Strings.Get("Alert for {0} was not found."), ipAddress));
        }
    }
